...
export default function Profile() {
  ...
  return (
    <div className="min-h-screen bg-[#f0f7f4] font-sans">
      {/* top bar */}
      <header className="border-b border-black/[0.06] bg-white px-8 py-4 flex items-center justify-between">
        <span className="text-[15px] font-semibold text-slate-800 tracking-tight">Focusync</span>
        <nav className="flex gap-6 text-sm text-slate-500">
          {['Beranda', 'Fokus', 'AI Chat', 'Check-in', 'Profil'].map((t) => (
            <a
              key={t}
              href="#"
              className={`transition hover:text-slate-800 ${t === 'Profil' ? 'text-emerald-600 font-medium' : ''}`}
            >
              {t}
            </a>
          ))}
      ...
      <main className="mx-auto max-w-5xl px-6 py-10 grid grid-cols-[300px_1fr] gap-8">
            ...
            <div className="px-5 pb-5 -mt-10">
              ...
              <div className="mt-3">
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  {user.role} · Terdaftar sejak {user.joined}
                </p>
              </div>
            </div>
          </div>
          ...
          {confirmReset && (
            <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 space-y-3">
              ...
              <div className="flex gap-2">
                <button className="flex-1 rounded-xl bg-rose-500 py-2 text-xs font-semibold text-white">
                  Ya, Reset
                </button>
                <button
